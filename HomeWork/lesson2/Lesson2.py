...
user = {}
for key, item in template.items(): #2:01
...
list(b)
[1, 3, 5]
Прямое преобразование к списку словаря возвращает список ключей
b.keys()
dict_keys([1, 3, 5])
...
ВОЗВРАЩЕНИЕ КОЛЛИЧЕСТВА ЕДИНИЦ В СПИСКЕ

a = [1, 2, 3, 4, 1, 2, 3, 4, 4, 4, 3]
a.count(1)
2
b = 'hello'
b.count('l')
2
a.index(3) первого встречного элемента (т.е. 2ое место с 0)
2
a.index(3, 3) последнее с какого места считать
6
...
for item in some_set:
    print(item)
#Не ко всем коллекциям и итерируемым обьектам можно обращаться по индексу, но все они интерируемы
#Не обязательно объект к которому можно обратиться по индексу будет итерируемым
"""
Хеширование - каждый обьект состоит из байт кода, если это статичный неизменяемый обьект то можно
вычислить спец алгоритмом спец хеш суммудля этого обьекта, что бы достоверно подтвердитьтот факт что выс качали то что положили в ппочку автор вы можете сверить хеш сумму
Математика вычисляет из совокупности байт сумму которая уникальна для этого файла в этом состоянии
Уникальный индентификатор это Хэш. Вероятность что хэш у 2х файлов будет абсолютноравен - возможно 
но близется к 0
Любое изменение файла приводит к тому что меняется Хэш сумма
